...
using Nover.DB.WebView2.Network;
...
namespace Nover.DB.WebView2.Defaults
{
    ...
    public partial class ResourceRequestHandler : IResourceRequestHandler
    {
        ...
        protected readonly ILogger<ResourceRequestHandler> _logger;
        ...
        public ResourceRequestHandler(IConfiguration config, 
            IScriptExecutor scriptExecutor, 
            IHostObjectProvider hostObjectHandler,
            IActionControllerProvider controllerProvider,
            IActionRouteProvider routeProvider,
            IDataTransferOptions dataTransferOptions,
            IServiceProvider serviceProvider,
            ILogger<ResourceRequestHandler> logger)
        {
            _config = config;
        ...
        */
        ...
        private void CacheAllRegisteredSchemeHandlers()
        {
            ...
            var customSchemeHandlers = _serviceProvider.GetServices(typeof(ISchemeHandler)) as IList<ISchemeHandler>;
            ...
            {
                var defaultSchemeHandlerObjs = _serviceProvider.GetServices(handlerType);
                ...
                if (CoreWebView2 != null)
                {
                    ...
                    if (!schemes.IsNullOrEmpty())
                    {
                        foreach (var scheme in schemes)
                        {
                            ...
                            {
                                _logger?.LogWarning($"Host name to folder map: folder: {folder} does not exist.");
                            }
                        }
                    }
                }
